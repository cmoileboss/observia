...
from data_enrichment.francetravail_api_call import (
    get_unique_rome_codes_from_csv_file,
    search_offres_by_rome,
)
from data_enrichment.import_formations_enriched import import_formations_enriched
from data_enrichment.create_output import create_output
from data_enrichment.formations_enricher import FormationsEnricher
from data_enrichment.sirene_enricher import fetch_siret, enrich
...
class Service:
    ...
    def get_formations_by_offre_id(
        ...
        """Retourne les formations liées au code ROME d'une offre."""
    ...
    def populate_database(self) -> None:
        ...
        if not (PROCESSED_DATA_ROOT / "merged_data.csv").exists():
            ...
            raise HTTPException(status_code=500, detail="Le fichier merged_data.csv est manquant. L'étape de création du fichier de données fusionnées et nettoyées a échoué.")
        if not (RAW_DATA_ROOT / "cdc_filtered_tech.csv").exists():
            logger.info("Le fichier cdc_filtered_tech.csv est manquant. Il s'agit du fichier contenant les informations de modalité par RNCP.")
            raise HTTPException(status_code=500, detail="Le fichier cdc_filtered_tech.csv est manquant. L'étape d'enrichissement des formations ne peut pas être effectuée.")
        if not (RAW_DATA_ROOT / "organismes_enriched.csv").exists():
            logger.info("Le fichier organismes_enriched.csv est manquant.")
            logger.info("Création du fichier organismes_enriched.csv à partir de merged_data.csv.")
            enrich(input_csv=str(PROCESSED_DATA_ROOT / "merged_data.csv"), output_csv=str(PROCESSED_DATA_ROOT / "organismes_enriched.csv"))
        if not (PROCESSED_DATA_ROOT / "formations_enriched.csv").exists():
            logger.info("Le fichier formations_enriched.csv est manquant. Début de l'enrichissement des formations.")
            ...
            logger.info("Enrichissement des formations terminé. Export vers formations_enriched.csv.")
        else:
        ...
        import_formations_enriched(self.db)
        ...
        for rome_code in rome_codes:
            search_offres_by_rome(rome_code, self.db)
