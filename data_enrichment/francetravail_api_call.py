...
def search_offres_by_rome(code_rome: str, db: Session):
    ...
    while True:
        ...
        if content_range:
            ...
            if total_count >= 3000:
                ...
                return get_offres_by_rome_and_departement(code_rome, db)
        ...
        if len(current_offers["resultats"]) < 150:
            ...
            populate_database_with_offres(current_offers["resultats"], db)
            break
        populate_database_with_offres(current_offers["resultats"], db)
...
def get_offres_by_rome_and_departement(code_rome: str, db: Session):
...
def populate_database_with_offres(offres, db):
    """Injection des offres dans la BDD.
    Args:
